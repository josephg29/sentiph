...
For each tentacle folder, read `agent.md` (its purpose/scope) and `todo.md` (its workload). Then evaluate the overall structure:
...
3. **Recommending drops without checking workload** — A tentacle with an empty `agent.md` but an active `todo.md` may need enrichment, not removal.
