      ...
      <div className="mx-auto max-w-3xl px-5 py-20 sm:px-6 sm:py-24">
        <div className="text-center">
          ...
          <p className="mx-auto mt-3 max-w-md text-[13.5px] leading-relaxed text-secondary">
            requires node 22+, pnpm, claude, and git. not on npm yet — clone and install globally.
          </p>
        </div>

        <div className="mx-auto mt-10 max-w-xl border border-border-strong bg-terminal-bg">
          <div className="flex items-center justify-between border-b border-white/10 px-3.5 py-2.5">
            <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-control text-white/55">
              <span className="h-1.5 w-1.5 animate-pulse-soft bg-term-green" aria-hidden />
              terminal
            </div>
            <CopyButton value={COMMAND} />
          </div>
          <pre className="overflow-x-auto p-4 font-mono text-[12.5px] leading-[1.75] text-white">
            {LINES.map((line, i) => (
            ...
            <div className="mt-2 text-term-green">↳ ui at http://localhost:8787</div>
          </pre>
        </div>

        <div className="mx-auto mt-6 flex max-w-xl flex-wrap justify-center gap-x-6 gap-y-1">
          {["node 22+", "pnpm", "claude cli", "git"].map((req) => (
            <span
              key={req}
              className="text-[10px] font-bold uppercase tracking-control text-muted"
            >
              · {req}
            </span>
          ))}
