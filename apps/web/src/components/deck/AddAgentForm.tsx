...
import { ACCESSORIES, ANIMATIONS, EXPRESSIONS, OCTOPUS_COLORS } from "./agentVisuals";

// ─── Add agent form ───────────────────────────────────────────────────────────

export type AgentAppearancePayload = {
  animation: string;
  expression: string;
  accessory: string;
  hairColor: string;
...
export const EXPRESSION_OPTIONS: { value: AgentExpression; label: string }[] = [
...
export const ACCESSORY_OPTIONS: { value: AgentAccessory; label: string }[] = [
...
export const AddAgentForm = ({
  ...
  const [selectedExpression, setSelectedExpression] = useState<AgentExpression>(() => {
    const pick = EXPRESSIONS[Math.floor(Math.random() * EXPRESSIONS.length)] as AgentExpression;
    return pick;
  });
  const [selectedAccessory, setSelectedAccessory] = useState<AgentAccessory>(() => {
    const pick = ACCESSORIES[Math.floor(Math.random() * ACCESSORIES.length)] as AgentAccessory;
    return pick;
  });
  const [selectedAnimation] = useState<AgentAnimation>(() => {
    const pick = ANIMATIONS[Math.floor(Math.random() * ANIMATIONS.length)] as AgentAnimation;
    return pick;
  });
