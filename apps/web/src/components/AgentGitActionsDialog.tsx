...
type AgentGitActionsDialogProps = {
  agentId: string;
  agentName: string;
...
export const AgentGitActionsDialog = ({
  agentId,
  agentName,
  gitStatus,
  gitPullRequest,
  ...
  return (
    <section
      aria-label={`Git actions for ${agentId}`}
    ...
    >
      ...
      <div className="git-actions-body">
        <p className="git-actions-message">
          Manage git lifecycle for <strong>{agentName}</strong> ({agentId}).
        </p>
        {isLoading ? (
        ...
        <section className="git-actions-commit-panel" aria-label="Source control composer">
          ...
          <textarea
            aria-label={`Commit message for ${agentId}`}
