...
type DeckAgentsDocument = {
  tentacles: Record<string, DeckAgentState>;
};

const readDeckState = (projectStateDir: string): DeckAgentsDocument => {
  ...
  try {
    ...
    if (
      ...
      return raw as DeckAgentsDocument;
    }
  } catch {
...
const writeDeckState = (projectStateDir: string, state: DeckAgentsDocument): void => {
  const filePath = join(projectStateDir, "state", "deck.json");
...
const parseAgentState = (raw: unknown, tentacleId: string): DeckAgentState => {
  const defaults: DeckAgentState = {
...
export const readDeckTentacles = (
  ...
  return Object.entries(deckState.tentacles).map(([tentacleId, raw]) => {
    const state = parseAgentState(raw, tentacleId);
    return {
      tentacleId,
