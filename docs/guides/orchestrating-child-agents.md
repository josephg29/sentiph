...
In worktree mode, each worker gets a branch named `sentiph/<worker-terminal-id>` under `.sentiph/worktrees/<worker-terminal-id>/` and is told to commit its work. The parent coordinator is responsible for merging branches, running tests, and updating session state.
