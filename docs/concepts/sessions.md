...
Sessions are the core abstraction in Sentiph.
...
- deleting a session removes its agent-facing files, but terminal records are separate runtime state
- restarting the API preserves terminal metadata and transcripts but not live PTYs
- channel messages do not persist, so durable handoffs belong in session markdown
- worktree branches are tied to worktree-backed terminals, not to the session folder itself
